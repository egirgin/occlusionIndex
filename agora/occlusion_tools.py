...
def occlusion_index_new(masks, keypoints):

    num_models = keypoints.shape[0]
    ...
    occluded_keypoints_list = np.zeros((num_models, num_joints), dtype=bool) # 1 means occluded, 0 means visible 
     
    truncated_keypoints_list = np.zeros_like(occluded_keypoints_list)
    ...
    for model_id in range(num_models): # this brings keypoints
        ...
        if person_unique_color == None: # sometimes it happens because full background mask exists in agora dataset
            # here we assume it is completely occluded, prob. true though
            occluded_keypoints_list[model_id] = np.ones(num_joints, dtype=bool)
            continue
        ...
        occlusion_status, truncation_status = occlusion_helper_new(all_mask=dilated_all_mask, keypoints=keypoints[model_id], model_color=all_mask_unique_color)

        occluded_keypoints_list[model_id] = occlusion_status
        truncated_keypoints_list[model_id] = truncation_status

    return occluded_keypoints_list, truncated_keypoints_list


def occlusion_helper_new(all_mask, keypoints, model_color):

    # keypoints: occluded 24x2
    # person color code: occluded

    imgHeight, imgWidth = all_mask.shape[:2] 

    neighbor_offset = 1

    num_joints = keypoints.shape[0]

    occluded_keypoints = np.zeros(num_joints, dtype=bool) # 1 means occluded, 0 means visible 
    truncated_keypoints = np.zeros(num_joints, dtype=bool) # 1 means occluded, 0 means visible 

    for kp_id, keypoint in enumerate(keypoints):

        if not (imgHeight > keypoint[1] > 0 or imgWidth > keypoint[0] >0):
            truncated_keypoints[kp_id] = True
            continue
        ...
        pixels = [
            ...
            [int(np.floor(keypoint[1])) - neighbor_offset, int(np.floor(keypoint[0]))], # up
            [int(np.floor(keypoint[1])) + neighbor_offset, int(np.floor(keypoint[0]))], # bottom

            [int(np.floor(keypoint[1])), int(np.floor(keypoint[0])) - neighbor_offset], # left
            ...
            #[int(np.floor(keypoint[1])) - neighbor_offset, int(np.floor(keypoint[0])) - neighbor_offset], # left - up
            #[int(np.floor(keypoint[1])) - neighbor_offset, int(np.floor(keypoint[0])) + neighbor_offset], # right - up
            #[int(np.floor(keypoint[1])) + neighbor_offset, int(np.floor(keypoint[0])) - neighbor_offset], # left - bottom
            ]
        ...
        if np.mean(np.absolute(keypoint_pixel_values - model_color), axis=1).all() != 0.0 :  # if the pixel value isn't the same
            occluded_keypoints[kp_id] = True


    return occluded_keypoints, truncated_keypoints
