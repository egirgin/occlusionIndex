...
def process_occlusion_data(occlusion_anno_path, category, weights, algorithm):
    ...
    for seq_name in seq_list:
        ...
        if seq_name in ["outdoors_freestyle_00", "courtyard_box_00"]:
            continue
        ...
        for frame_id, json_filename in enumerate(annotation_list):
            ...
            for model_id in list(annotation_dict.keys()):
                ...
                if algorithm != "crowd_index" and "truncation" in annotation_dict[model_id].keys() and annotation_dict[model_id]["truncation"]["overall"] > 0.75:
                    continue
                else:
                    if algorithm == "regional_occlusion":
                        for joint_id, joint_name in enumerate(annotation_dict[model_id][category].keys()):                      
                            ...
                            if "truncation" in annotation_dict[model_id].keys():
                                ...
                                occlusion_score += weights[joint_id] * (body_segment_occlusion/100)
                        ...
                        for joint_id, joint_name in enumerate(annotation_dict[model_id]["occlusion"].keys()): 
                            ...
                            if "truncation" in annotation_dict[model_id].keys():
                                ...
                                occlusion_score += weights[joint_id] * float(body_segment_occlusion) 
...
def self_occlusion(occlusion_anno_path, weights):
    seq_list = ["outdoors_freestyle_00"]
    ...
    for seq_name in seq_list:
        ...
        for frame_id, json_filename in enumerate(annotation_list):
            ...
            for model_id in list(annotation_dict.keys()):
                ...
                for joint_id, joint_name in enumerate(annotation_dict[model_id]["self_occlusion"].keys()):                      
                    body_segment_self_occlusion = annotation_dict[model_id]["self_occlusion"][joint_name]
                    occlusion_score += weights[joint_id] * (body_segment_self_occlusion/100)
